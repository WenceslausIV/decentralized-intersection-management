        ...
        for j in range(i+1, N):
            ...
            if adifference > bdifference:
                for k in range(50):
                    #print("there")
                    #print(indices)
                    steplength = abs(biga - smalla) / 50
                    step_a = smalla + steplength * k
                    def equation5(b):
                        term1 = np.sqrt((step_a- x[0, i])**2 + (b - x[1, i])**2) - ci
                        term2 = np.sqrt((step_a- x[0, j])**2 + (b - x[1, j])**2) - cj
                        term3 = np.sqrt((clist[i].center.x - x[0, i])**2 + (clist[i].center.y - x[1, i])**2) - ci
                        term4 = np.sqrt((clist[i].center.x - x[0, j])**2 + (clist[i].center.y - x[1, j])**2) - cj
                        term5 = np.sqrt((clist[j].center.x - x[0, i])**2 + (clist[j].center.y - x[1, i])**2) - ci
                        term6 = np.sqrt((clist[j].center.x - x[0, j])**2 + (clist[j].center.y - x[1, j])**2) - cj
