...
c2 = Car(Point(118,68), np.pi, 'blue')
    ...
    for i in range(N):
        for j in range(i+1, N):
            ...
            if lane_list[i] == lane_list[j]:
                ...
                if lane_list[i] == 'd': # assuming d passes the conflict zone first
                    #print('down car exists')
                    constraint_value3 = (1 / phi) * (lambda_4 * ((70.0-x[1,i]) + ( -(70.0-x[0,j]) ) - (gamma2 + phi * v_i)) - (v_i + v_j) ) + F_rm_i
                    A[count, j] = 1.0
                    b[count] = constraint_value3
                    count += 1
                elif lane_list[j] == 'd' and x[0,i] > 68:
                    #print('down car exists as j')
                    constraint_value3 = (1 / phi) * (lambda_4 * ((70.0-x[1,j]) + ( -(70.0-x[0,i]) ) - (gamma2 + phi * v_j)) - (v_j + v_i) ) + F_rm_i
                    A[count, j] = 1.0
                    b[count] = constraint_value3
                    count += 1
...
pid_controller = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=3.0)
