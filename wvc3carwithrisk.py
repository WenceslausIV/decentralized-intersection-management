...
c1 = Car(Point(100,68), np.pi, 'black')
c1.velocity = Point(0,0)
w.add(c1)
current_heading = c1.heading
#print("Current heading:", current_heading)

c2 = Car(Point(54, 90), -np.pi/2, 'blue')
...
pid_controller = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=4.0)
pid_controller2 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity = 3.0)
...
if not human_controller:
    ...
    while time_passed < 800.0:  # Run simulation for 40 seconds
        ...
        if time_passed < 710:
            ...
            cp1 = wvcontrolr2l(pointlist_r, 0)
            if cp1 == None:
                gp1 = 40
            else:
                gp1 = np.linalg.norm(x[:,0] - np.array([cp1[0], cp1[1]]))
            k1 = 0.1

            cp2 = wvcontrolu2d(pointlist_r, 1)
            if cp2 == None:
                gp2 = 40
            else:
                gp2 = np.linalg.norm(x[:,1] - np.array([cp2[0], cp2[1]]))
            k2 = 0.1
            ...
            if cp3 == None:
                ...
                gp3 = np.linalg.norm(x[:,2] - np.array([cp3[0], cp3[1]]))
            k3 = 0.1

            #cp4 = wvcontroll2r(pointlist_r, 3)
            #if cp4 == None:
            #    gp4 = 40
            #else:
            #    gp4 = np.linalg.norm(x[:,3] - np.array([cp4[0], cp4[1]]))
            #k4 = 0.1
