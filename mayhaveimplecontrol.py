...
if not human_controller:
    ...
    while time_passed < 800.0:  # Run simulation for 40 seconds
        ...
        if time_passed < 710:
            ...
            wv()
            cp1 = wvcontrol(pointlist_r, 1)
            gp = np.linalg.norm(x[:,1] - np.array([cp1[0], cp1[1]])) -2 #2 is car length /2
            k = 0.2
            print(k * gp)
            ...
            pid_controller2 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity = k * gp)
