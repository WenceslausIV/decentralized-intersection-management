...
c1 = Car(Point(25,120), -np.pi/2, 'black')
c1.velocity = Point(0,0)
w.add(c1)
current_heading = c1.heading
#print("Current heading:", current_heading)

c2 = Car(Point(60, 120), -np.pi/2, 'blue')
c2.velocity = Point(0,0) 
w.add(c2)

c3 = Car(Point(85, 120), -np.pi/2, 'green')
c3.velocity = Point(0,0) 
w.add(c3)

c4 = Car(Point(25, 0), np.pi/2, 'yellow')
c4.velocity = Point(0,0) 
w.add(c4)

c5 = Car(Point(60, 0), np.pi/2, 'brown')
c5.velocity = Point(0,0) 

w.add(c5)

c6 = Car(Point(85, 0), np.pi/2, 'pink')
c6.velocity = Point(0,0) 

w.add(c6)

c7 = Car(Point(0, 60), 0, 'purple')
...
pointlist_r_clone = []
def wv():
    print('x', c7.velocity.x)
    print('y', c7.velocity.y)
    ...
    #print("car 1", np.sqrt(clist[0].velocity.x**2 + clist[0].velocity.y**2))
    #print("car 2", np.sqrt(clist[1].velocity.x**2 + clist[1].velocity.y**2))   
    #print("car 3", np.sqrt(clist[2].velocity.x**2 + clist[2].velocity.y**2))
    #print("car 7", np.sqrt(clist[6].velocity.x**2 + clist[6].velocity.y**2))
    #print("car 7", clist[6].velocity.x**2 , clist[6].velocity.y**2)
    gamma = 1
    safety_radius = 4
    c= 100000
    risk = np.zeros(N)
    ...
    for i in range(N-1):
        for j in range(i+1, N):
            ...
            def equation1(b):
                term1 = abs(-( 2*(0 - x[0,i]) * (0 - clist[i].velocity.x) + 2 *(b - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - 0)**2 + (x[1,i] - b)**2 - safety_radius**2) - c  ) - risk[i]
                term2 = abs(-( 2*(0 - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(b - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - 0)**2 + (x[1,j] - b)**2 - safety_radius**2) - c   ) - risk[j]
                #print("term1", term1)
                return term1 - term2
            ...
            def equation2(b):
                term1 = abs(-( 2*(120 - x[0,i]) * (0 - clist[i].velocity.x) + 2 *(b - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - 120)**2 + (x[1,i] - b)**2 - safety_radius**2) - c ) - risk[i]
                term2 = abs(-( 2*(120 - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(b - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - 120)**2 + (x[1,j] - b)**2 - safety_radius**2) - c  ) - risk[j]
                return term1 - term2
            ...
            def equation3(a):
                term1 = abs(-( 2*(a - x[0,i]) * (0 - clist[i].velocity.x) + 2* (0 - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - a)**2 + (x[1,i] - 0)**2 - safety_radius**2) - c ) - risk[i]
                term2 = abs(-( 2*(a - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(0 - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - a)**2 + (x[1,j] - 0)**2 - safety_radius**2) - c ) - risk[j]
                return term1 - term2
            ...
            def equation4(a):
                term1 = abs(-( 2*(a - x[0,i]) * (0 - clist[i].velocity.x) + 2* (120 - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - a)**2 + (x[1,i] - 120)**2 - safety_radius**2) - c ) - risk[i]
                term2 = abs(-( 2*(a - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(120 - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - a)**2 + (x[1,j] - 120)**2 - safety_radius**2) - c ) - risk[j]
                return term1 - term2
            a_initial_guess = 60.0
            ...
            if adifference > bdifference:
                for k in range(100):
                    ...
                    def equation5(b):
                        term1 = abs(-( 2*(step_a - x[0,i]) * (0 - clist[i].velocity.x) + 2 *(b - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - step_a)**2 + (x[1,i] - b)**2 - safety_radius**2) - c ) - risk[i]
                        term2 = abs(-( 2*(step_a - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(b - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - step_a)**2 + (x[1,j] - b)**2 - safety_radius**2) - c ) - risk[j]
                        print("term1", term1)
                        return term1 - term2
                ...
                for k in range(100):
                    ...
                    def equation6(a):
                        term1 = abs(-( 2*(a - x[0,i]) * (0 - clist[i].velocity.x) + 2* (step_b - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - a)**2 + (x[1,i] - step_b)**2 - safety_radius**2) - c  ) - risk[i]
                        term2 = abs(-( 2*(a - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(step_b - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma * (( x[0,j] - a)**2 + (x[1,j] - step_b)**2 - safety_radius**2) - c  ) - risk[j]
    ...
    for i in range(1):
        ...
        for j in range(N):
            if i != j:
                ...
                term3 = abs(-( 2*(x[0,i] - x[0,i]) * (0 - clist[i].velocity.x) + 2* (x[1,i] - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - x[0,i])**2 + (x[1,i] - x[1,i])**2 - safety_radius**2) - c ) - risk[i]
                term4 = abs(-( 2*(x[0,i] - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(x[1,i] - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - x[0,i])**2 + (x[1,j] - x[1,i])**2 - safety_radius**2) - c ) - risk[j]
                

                if (term3 - term4) > 0:

                    for v in range(len(pointlist_r)):
                        term3 = abs(-( 2*(pointlist_r[v][0] - x[0,i]) * (0 - clist[i].velocity.x) + 2* (pointlist_r[v][1] - x[1,i]) * (0 - clist[i].velocity.y) ) - gamma * (( x[0,i] - pointlist_r[v][0])**2 + (x[1,i] - pointlist_r[v][1])**2 - safety_radius**2)- c ) - risk[i]
                        term4 = abs(-( 2*(pointlist_r[v][0] - x[0,j]) * (0 - clist[j].velocity.x) + 2 *(pointlist_r[v][1] - x[1,j]) * (0 - clist[j].velocity.y) ) - gamma* (( x[0,j] - pointlist_r[v][0])**2 + (x[1,j] - pointlist_r[v][1])**2 - safety_radius**2) - c ) - risk[j]
...
pid_controller = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=10.0)
pid_controller2 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity = 10.0)
pid_controller3 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity= 10.0)
pid_controller4 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=10.0)
pid_controller5 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=10.0)
pid_controller6 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=10.0)
pid_controller7 = PIDController(kp=1.0, ki=0.5, kd=0.1, target_velocity=20.0)
#############################################################################
...
if not human_controller:
    ...
    while time_passed < 800.0:  # Run simulation for 40 seconds
        ...
        if time_passed < 710:
            ...
            u_ref = np.array([throttle1 ,throttle2, throttle3,throttle4 ,throttle5, throttle6,throttle7])
